...
- 프롬프트:
  ...
  매주 월요일 또는 마지막 심층 리뷰 후 7일이 지났으면 신규 변경, 운영 추세, 구매자·사진가·
  관리자 사용자 여정과 반복 업무를 사람처럼 종합 검토해 개선 후보도 최대 3건 제안하세요.
...
한국시간 월요일이거나 automation memory에 기록된 마지막 심층 리뷰로부터 7일이 지났으면
일일 오류 점검에 더해 다음 제품 리뷰를 수행한다. 실행 뒤 기준 시각과 main SHA를 memory에
`weekly-product-review` 표식으로 기록해 같은 주에 반복하지 않는다.

1. 마지막 심층 리뷰 이후의 main 커밋·PR·문서 변경을 읽고 실제 사용자·운영 흐름에 미친 영향을 확인한다.
2. 구매자의 탐색·검색·문의·주문·다운로드, 사진가의 가입·승인·업로드·검토 결과 확인,
   관리자의 이미지 검토·문의·촬영 의뢰·입금·오류 대응 여정을 공개 화면과 코드로 따라간다.
3. 최근 7일과 30일의 개인정보 없는 활동·대기·실패 추세를 비교하고 반복 수작업, 막힌 흐름,
   설명 부족, 발견성·접근성 문제와 불필요한 비용을 찾는다.
4. `system-definition`, 운영 백로그, 열린 이슈, 최근 지원 유형과 Grok·Gemini 반론을 대조하되
   기존 목록을 그대로 반복하지 않고 현재 근거가 남아 있는지 재확인한다.
5. 오류가 아니어도 사용자 가치, 운영 시간 절감, 전환·콘텐츠 품질 또는 위험 감소가 명확한
   개선을 후보로 제안한다. 사실과 추론을 구분하고 추론만 있는 아이디어는 이슈로 만들지 않는다.

주간 심층 리뷰는 새 후보를 최대 3건만 만든다. 각 후보에는 관찰 근거, 영향을 받는 역할과
여정, 예상 효과, 신뢰도, 구현 난이도, 부작용·반론, 검증 방법과 완료 기준을 기록한다.
기존 이슈와 의미가 겹치면 새 이슈 대신 기존 이슈에 최신 근거를 추가한다. 자동 구현·승인·
병합·배포는 하지 않는다.
