...
flowchart LR
  ...
  D --> E["search_administrative_areas"]
  E --> F["최대 8개 계층 주소 반환"]
```

검색 API는 공개 DB 조회를 허용하지 않는다. 애플리케이션 API에서 로그인 사용자와 승인 작가 여부를 확인한 후에만 service role로 RPC를 호출한다. IP당 분당 120회 제한이 적용된다.
...
5. 비로그인 요청은 401, 과도한 요청은 429인지 확인
