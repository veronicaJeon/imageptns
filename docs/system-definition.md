...
- 문장 검색은 제목·태그를 최우선, 설명·AI 보조 캡션을 다음, 카테고리를 보조 신호로 평가한다. 한국어 두 글자 이상 검색 토큰은 접두 일치로 조사와 결합어를 포괄하므로 `북단`은 `북단의`, `한강`은 `한강공원`을 찾는다. 한 글자 검색은 과도한 확장을 막기 위해 정확히 일치시킨다. 여러 검색 토큰은 모두 포함하는 AND 조건을 유지한다. 임계값을 넘는 키워드 결과가 있으면 해당 결과만 점수순으로 제공하고 외부 임베딩 공급자를 호출하지 않는다. 키워드가 약할 때만 의미 검색 후보가 될 수 있으며, 두 점수가 모두 임계값 미만이면 결과를 제공하지 않는다.
...
6. 주문 접수·승인·취소 메일은 outbox에 기록한 뒤 발송한다. 계좌이체 요청이 생성되면 운영 수신함에도 입금 확인 요청 메일을 보내고, 메일 장애는 거래를 되돌리지 않고 실패 상태와 재전송 대상으로 남긴다.
7. 관리자는 상단 `입금 확인 요청` 대기열과 미처리 건수 배지에서 요청을 확인하고, 실제 입금을 확인해 주문을 확정한다. 24시간을 넘긴 요청은 대기열에서 강조한다.
...
- 2026-08-27 계좌이체 주문의 관리자 입금 확인 요청을 상단 핵심 대기열과 미처리 건수 배지로 승격하고, 기존 거래 outbox 기반 운영 수신함 메일 경로를 회귀 테스트로 고정했다.
- 2026-08-28 텍스트 검색 route에서 이미지 파생본용 Sharp가 우발적으로 로드되는 의존성을 분리하고, 방향 검색어의 구조화된 `orientation_class` 필터를 유지했다. 검색 요청 실패는 무결과와 구분해 표시하고 수동 Production 배포에서 `한강` 키워드 결과를 회귀 확인한다.
- 2026-08-31 PostgreSQL `simple` 검색이 한국어 조사를 분리하지 못해 `북단`이 설명의 `북단의`를 찾지 못하는 문제를 한국어 두 글자 이상 접두 토큰 검색으로 보완했다. 한 글자·비한국어 검색과 다중 단어 AND 조건은 유지한다.
