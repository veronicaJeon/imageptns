...
- 카탈로그 사진의 외부 임베딩 생성은 업로드 완료가 아니라 관리자 승인으로 `approved + active + is_published=true`가 확정된 뒤에만 시작한다. 반려·취소될 업로드를 공급자에게 보내지 않는다.
...
## 공개 텍스트 검색의 키워드 우선 정책

공개 라이브러리의 한국어·영어 문장 검색은 점수를 단순 가중합하지 않고 단계식으로 처리한다.

1. 제목·태그(A), 설명·캡션(B), 카테고리(C)의 weighted FTS 관련도를 먼저 계산한다.
2. `KEYWORD_SEARCH_STRONG_THRESHOLD` 이상의 결과가 하나라도 있으면 관련도순으로 그 결과만 반환하고 외부 임베딩 API를 호출하지 않는다.
3. 강한 키워드 결과가 없을 때만 같은 문장을 query 임베딩으로 바꾸고 공개 카탈로그 벡터를 검색한다.
4. cosine similarity가 `SEMANTIC_SEARCH_MIN_SIMILARITY` 미만인 결과는 노출하지 않는다. 키워드와 임베딩이 모두 기준 미달이면 빈 결과를 반환한다.
5. 키워드·의미 경로 모두 승인·활성·공개 상태와 카테고리·방향·사용조건 필터를 다시 검사한다.

초기 기본값은 키워드 `0.12`, semantic `0.72`다. 현재 소규모 연결 시험에서 정한 보수적 시작값이며, 검색 원문을 기록하지 않는 집계와 정답셋 평가로만 조정한다.

사진 입력 검색은 SHA-256 exact와 pHash·dHash 근접 결과를 먼저 반환한다. 지문 결과가 없을 때만 image query 임베딩 fallback을 허용하고 동일 semantic 하한을 적용한다.

## 백필·전환·롤백

1. 신규 이미지는 관리자 승인 성공 후 새 모델 조합의 행을 `pending`으로 멱등 생성한다. 승인 API는 외부 공급자를 호출하지 않는다.
2. 별도 비동기 작업자가 원자적 claim token으로 `pending` 행을 가져와 공개 미리보기 파생본으로 임베딩하고 각 행을 독립적으로 `ready`, `failed` 또는 `stale`로 기록한다. 기본 한 번에 1개, 최대 3개를 순차 처리한다.
...
기능 활성화 전에 이미 승인된 기존 카탈로그는 같은 자격 조건을 검사하는 별도 cursor 백필로 등록한다. 업로드·검토 대기·반려·취소 이미지는 백필 대상에서도 제외한다.

운영 플래그는 master인 `SEMANTIC_IMAGE_SEARCH_ENABLED` 아래 `SEMANTIC_IMAGE_INDEXING_ENABLED`와 `SEMANTIC_IMAGE_QUERY_ENABLED`를 분리한다. 코드·DB를 먼저 배포해도 세 플래그가 명시적으로 켜지지 않으면 승인 큐, worker와 사용자 query가 외부 공급자를 호출하지 않는다.
