...
| P1 | 안전한 공개 검색 구현·Voyage 활성화 보류 | 사진으로 검색·이미지 캡션 | 대화형 검색 1.5초, 비동기 캡션 5초 예산을 확정했다. 검색 후보는 Voyage 512d와 Nemotron Embed 2048d로 압축하고 image→image는 Voyage만 지원한다. 느리거나 가용하지 않은 reranker·VLM은 실행 후보에서 제외했다. 공개 검색은 제목·태그 A, 설명·캡션 B, 카테고리 C의 강한 키워드 결과를 우선하고 없을 때만 semantic fallback하며 양쪽 임계값 미달이면 0건이다. 신규 카탈로그 임베딩은 관리자 승인 후 pending 작업만 등록하고 원자적 worker가 기본 1개씩 처리한다. 사용자가 Voyage 결제수단·학습 opt-out을 완료하고 방침 변경을 승인했으나 Production 키와 처리 국가·DPA·하위처리자 서면 근거가 아직 없어 실제 작가 사진 백필·query 플래그는 OFF로 유지한다. 비개인 canary와 본선 평가 후 단계적으로 활성화한다. |
