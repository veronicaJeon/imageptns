...
| P1 | 개선 루프 구축·에이전트 연결 대기 | 72시간 유지보수 루틴 | 검사·백로그에서 고정 ID 후보 이슈를 만들고 승인·거절, 해결 자동 종료, 승인된 수동 배포 결과를 누적하도록 보강했다. 최초 workflow 실행을 검증하고, 지속적인 코드 구현을 위해 Codex scheduler/API webhook을 연결한다. |
