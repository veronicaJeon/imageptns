...
| P1 | 운영 전환 진행·백필 관찰 필요 | 사진으로 검색·이미지 캡션 | Voyage 운영 키, 결제수단, 학습 사용 거부와 공개 처리방침 반영 뒤 승인 시 큐잉·일일 백필·한국어 문장 및 사진 의미 fallback을 배포한다. 백필 ready/failed/stale 수와 임계값별 무결과율을 관찰한다. NVIDIA 비동기 캡션은 코드와 5초 제한을 마련했지만 Build 무료 Trial은 운영 사용이 금지되므로 운영 구독 또는 self-host 권한 확인 전 `NVIDIA_CAPTIONING_ENABLED=false`를 유지한다. 6초대 재정렬은 대화형 예산 초과로 제외한다. |
