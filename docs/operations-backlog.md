...
| P0 | 코드 수정·배포 검증 대기 | 공개 키워드 검색 런타임 | 텍스트 검색 route가 이미지 가공 모듈을 통해 Sharp를 불필요하게 로드해 운영 500이 발생했다. 방향 키워드 필터는 보존하고 경량 버전 모듈로 의존성을 분리했으며, 검색 실패 표시와 `한강` 운영 배포 스모크를 추가했다. 배포 후 `한강` 및 `한강 세로사진`을 확인한다. |
