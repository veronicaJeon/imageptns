...
| P0 | 런타임 복구 완료·한국어 검색 배포 검증 대기 | 공개 키워드 검색 | 텍스트 검색 route의 불필요한 Sharp 의존성을 분리해 운영 500을 복구했다. PostgreSQL `simple` 검색이 설명의 `북단의`를 조사 없는 `북단`으로 찾지 못하는 문제는 한국어 두 글자 이상 접두 토큰 검색으로 보완했다. 배포 후 `한강`, `한강 세로사진`, `한강 북단`을 확인한다. |
