...
| P1 | 1단계 구현·배포 전 검증 | 사진으로 검색 | 중복 탐지 지문을 재사용하는 비저장 근접 사진 검색 API와 라이브러리 UI를 구현했다. 브라우저에서 25MB 이하 원본을 EXIF 없는 3.5MB 이하 검색본으로 축소해 Vercel 4.5MB 본문 한도 안에서 처리한다. 운영 반영 전 공개 상태 누출·크기·분당 20회 제한을 회귀하고, 지문 보유 공개 이미지가 1,500건에 이르면 DB 거리 계산으로 이전한다. 의미 기반 검색은 공급자·국외 이전·보유·비용·재색인 정책 승인 후 별도 도입한다. |
