...
import { IMAGE_REVIEW_STATUS_LABELS, KO_SERVICE_TERMS } from "@/lib/ux/terminology";
...
const STATUS_OPTIONS: Array<{ value: CleanupStatus; label: string }> = [
  { value: "all", label: IMAGE_REVIEW_STATUS_LABELS.all },
  { value: "draft", label: IMAGE_REVIEW_STATUS_LABELS.draft },
  { value: "pending", label: IMAGE_REVIEW_STATUS_LABELS.pending },
  { value: "approved", label: IMAGE_REVIEW_STATUS_LABELS.approved },
  { value: "rejected", label: IMAGE_REVIEW_STATUS_LABELS.rejected },
];
...
export default function AdminImageCleanupPage() {
  ...
  async function purgeSelected() {
    if (selectedIds.length === 0 || confirmation !== KO_SERVICE_TERMS.permanentDeletion) return;
    ...
    try {
      ...
      setLastResult(`완전삭제 ${data?.summary?.purged ?? 0}개, 실패 ${data?.summary?.failed ?? 0}개`);
      setConfirmation("");
    ...
    <div className="p-6 md:p-10">
      <div className="mb-8">
        ...
        <p className="mt-1 max-w-3xl text-sm leading-relaxed text-outline">
          ...
          판매, 다운로드, 정산, 온체인/Arweave, 소싱 결과, 삭제 요청 이력이 있으면 완전삭제 후보에서 제외됩니다.
        </p>
      </div>
      ...
      <div className="mb-4 flex flex-col gap-3 rounded-xl bg-surface-container-lowest p-4 shadow-ghost lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-outline">삭제 전 확인 문구에 <span className="font-bold text-on-surface">완전삭제</span>를 입력해야 실행됩니다.</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <input
            ...
            placeholder={KO_SERVICE_TERMS.permanentDeletion}
          />
          <button
            type="button"
            onClick={purgeSelected}
            disabled={selectedIds.length === 0 || confirmation !== KO_SERVICE_TERMS.permanentDeletion || purging}
        ...
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody className="divide-y divide-outline-variant/20">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-5 py-16 text-center text-outline">조건에 맞는 완전삭제 후보가 없습니다.</td></tr>
